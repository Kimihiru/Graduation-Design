    ...
    <el-container>
      ...
      <el-container>
        ...
        <el-main>
          ...
          <el-dialog title="用户信息" :visible.sync="dialogFormVisibleForDelete" width="30%">
            <el-form label-width="80px" size="small">
...
export default {
  ...
  methods: {
    ...
    load() {
      //请求分页查询数据
      fetch("http://localhost:8080/page?pageNum="+this.pageNum
        +"&pageSize="+this.pageSize
        +"&username="+this.username+"").then(res=>res.json()).then(res=>{
        console.log(res)
    ...
    insert(){
      axios.post("/saveOrUpdate",this.form).then(res=>{
        if(res.data==true){
          alert("保存成功");
    ...
    deleteById(row){
      ...
      confirm("确定删除?");
      axios.post("/delete",this.form).then(res=>{
        if(res.data==true){
          alert("删除成功");
          this.load();
        }
